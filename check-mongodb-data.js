#!/usr/bin/env node
/**
 * Check MongoDB Collection Data - Fixed Version
...
async function analyzeCollectionData() {
  try {
    ...
    const db = resultsConn.db;
    const collection = db.collection(process.env.QUIZ_COLLECTION || 'school students');
    ...
    if (totalCount === 0) {
      console.log('📭 Collection is empty - no data found');
